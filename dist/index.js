...
var getConfigPath = ({
  ...
  return import_path2.default.resolve(getProjectRoot(), configPath);
};
...
var getSelectorsCount = (bodyNode) => {
  ...
  const checkNode = (node) => {
    if (node.type === import_utils12.TSESTree.AST_NODE_TYPES.ClassDeclaration)
      incrementSelectorCount("class");
    if (node.type === import_utils12.TSESTree.AST_NODE_TYPES.FunctionDeclaration)
      incrementSelectorCount("function");
    if (node.type === import_utils12.TSESTree.AST_NODE_TYPES.TSTypeAliasDeclaration)
      incrementSelectorCount("type");
    if (node.type === import_utils12.TSESTree.AST_NODE_TYPES.TSInterfaceDeclaration)
      incrementSelectorCount("interface");
    if (node.type === import_utils12.TSESTree.AST_NODE_TYPES.TSEnumDeclaration)
      incrementSelectorCount("enum");
    if (node.type === import_utils12.TSESTree.AST_NODE_TYPES.VariableDeclaration)
      node.declarations.forEach((variableNode) => {
        if (variableNode.init?.type === import_utils12.TSESTree.AST_NODE_TYPES.ArrowFunctionExpression)
  ...
  bodyNode.forEach((node) => {
    if (node.type === import_utils12.TSESTree.AST_NODE_TYPES.ClassDeclaration || node.type === import_utils12.TSESTree.AST_NODE_TYPES.FunctionDeclaration || node.type === import_utils12.TSESTree.AST_NODE_TYPES.TSTypeAliasDeclaration || node.type === import_utils12.TSESTree.AST_NODE_TYPES.TSInterfaceDeclaration || node.type === import_utils12.TSESTree.AST_NODE_TYPES.TSEnumDeclaration || node.type === import_utils12.TSESTree.AST_NODE_TYPES.VariableDeclaration)
      checkNode(node);
    if ((node.type === import_utils12.TSESTree.AST_NODE_TYPES.ExportNamedDeclaration || node.type === import_utils12.TSESTree.AST_NODE_TYPES.ExportDefaultDeclaration) && (node.declaration?.type === import_utils12.TSESTree.AST_NODE_TYPES.ClassDeclaration || node.declaration?.type === import_utils12.TSESTree.AST_NODE_TYPES.FunctionDeclaration || node.declaration?.type === import_utils12.TSESTree.AST_NODE_TYPES.TSTypeAliasDeclaration || node.declaration?.type === import_utils12.TSESTree.AST_NODE_TYPES.TSInterfaceDeclaration || node.declaration?.type === import_utils12.TSESTree.AST_NODE_TYPES.TSEnumDeclaration || node.declaration?.type === import_utils12.TSESTree.AST_NODE_TYPES.VariableDeclaration))
...
// src/rules/fileComposition/fileComposition.ts
var fileComposition = import_utils13.ESLintUtils.RuleCreator(
...
var createProjectStructureCacheFile = ({
  ...
  const filePath = import_path6.default.join(projectRoot, PROJECT_STRUCTURE_CACHE_FILE_NAME);
  const jsonData = JSON.stringify(projectStructureCache, null, 2);
  (0, import_fs2.writeFileSync)(filePath, jsonData, "utf8");
};
...
var readProjectStructureCacheFile = (projectRoot) => {
  try {
    return JSON.parse(
      (0, import_fs3.readFileSync)(
        import_path7.default.join(projectRoot, PROJECT_STRUCTURE_CACHE_FILE_NAME),
        "utf-8"
      )
    );
  } catch (_e) {
...
var cleanUpErrorFromCache = ({
  ...
  const projectStructureCacheClean = projectStructureCache.filter(
    (cache) => filename !== cache.filename && (0, import_fs4.existsSync)(cache.filename)
  );
  if (!projectStructureCacheClean.length)
    return (0, import_fs4.unlinkSync)(
      import_path8.default.join(projectRoot, PROJECT_STRUCTURE_CACHE_FILE_NAME)
    );
...
var handleCache = ({
  ...
  const projectStructureCacheClean = projectStructureCache.filter(
    ({ filename }) => (0, import_fs5.existsSync)(filename)
  );
...
var getNodePathWithStructureRoot = ({
  nodePath,
  structureRoot
}) => import_path9.default.join(structureRoot ?? "", nodePath).replaceAll(import_path9.sep, "/");
...
var checkNodeExistence = ({
  ...
  const nodeDirname = import_path10.default.dirname(nodePath);
  ...
  const enforcedNodeNames = currentEnforceExistence.map((enforcedNodeName) => {
    ...
    const enforcedNodeFullPath = import_path10.default.join(
      structureRoot,
      currentDirname,
      enforcedNodeNameWithoutRef
    );
    if (import_fs6.default.existsSync(enforcedNodeFullPath)) return;
    return "./" + import_path10.default.relative(projectRoot, enforcedNodeFullPath).replaceAll(import_path10.sep, "/");
  }).filter((v) => v !== void 0);
...
var getPathname = ({ root, filename }) => import_path11.default.relative(root, filename).replaceAll(import_path11.sep, "/");
...
var validateLongPath = ({
  ...
  const currentPath = longPathsInfo?.countFromSystemRoot ? filename : getPathname({
    root: import_path12.default.resolve(projectRoot, longPathsInfo?.root ?? ".."),
    filename
  });
...
var validateFolderStructure = ({
  ...
  const rootFolderName = import_path13.default.basename(structureRoot);
...
var handleProgram = ({
  ...
  const structureRoot = import_path14.default.resolve(projectRoot, config.structureRoot ?? ".");
...
// src/rules/folderStructure/folderStructure.ts
var folderStructure = import_utils14.ESLintUtils.RuleCreator(
...
var getPathAliases = ({
  ...
  const tsconfigPath = config.tsconfigPath ? import_path15.default.resolve(projectRoot, config.tsconfigPath) : import_path15.default.join(projectRoot, "tsconfig.json");
  let tsconfig;
  try {
    tsconfig = (0, import_comment_json2.parse)(
      (0, import_fs7.readFileSync)(tsconfigPath, "utf-8")
    );
  } catch (_e) {
    return;
  }
...
var getFullImportPathVariants = ({
  ...
  const fullImportPath = import_path16.default.join(projectRootWithBaseUrl, importPath);
  const fullImportPathIndex = import_path16.default.join(fullImportPath, "index");
  const fullImportPathExternal = import_path16.default.join(
    projectRoot,
    "node_modules",
    importPath
  );
  const fullImportPathExternalIndex = import_path16.default.join(
    fullImportPathExternal,
    "index"
  );
  const fullImportPathExternalTypes = import_path16.default.join(
    projectRoot,
    "node_modules",
    "@types",
    importPath
  );
  const fullImportPathExternalTypesIndex = import_path16.default.join(
    fullImportPathExternalTypes,
    "index"
  );
  const fullImportPathExternalNode = import_path16.default.join(
    projectRoot,
    "node_modules",
    "node",
    importPath
  );
  const fullImportPathExternalNodeIndex = import_path16.default.join(
    fullImportPathExternalTypes,
    "index"
  );
  const fullImportPathExternalTypesNode = import_path16.default.join(
    projectRoot,
    "node_modules",
    "@types",
    "node",
  ...
  const fullImportPathExternalTypesNodeIndex = import_path16.default.join(
    fullImportPathExternalTypes,
    "index"
  );
  return {
  ...
  const importPathWithExtension = allExtensions.reduce(
    (acc, ext) => {
      const isImportPathWithoutIndex = fullImportPathsWithoutIndex.some(
        (fullImportPathWithoutIndex) => import_fs8.default.existsSync(fullImportPathWithoutIndex + ext)
      );
      if (isImportPathWithoutIndex) return acc = importPath + ext;
      const isImportPathWithIndex = fullImportPathsWithIndex.some(
        (fullImportPathWithIndex) => import_fs8.default.existsSync(fullImportPathWithIndex + ext)
      );
...
var getDirnamePath = (fileName, pattern) => {
  ...
  for (let i = 0; i < lvl; i++) {
    dirnamePath = import_path17.default.dirname(dirnamePath);
  }
  return dirnamePath;
};
...
var isExternalImport = (importPath, projectRoot) => {
  ...
  return importPaths.some((iPath) => {
    ...
    return import_fs9.default.existsSync(fullImportPathExternal) || import_fs9.default.existsSync(fullImportPathExternalTypes) || import_fs9.default.existsSync(fullImportPathExternalNode) || import_fs9.default.existsSync(fullImportPathExternalTypesNode);
  });
};
...
var isImportPathExists = ({
  ...
  projectRoot,
  baseUrl
}) => import_fs10.default.existsSync(import_path18.default.join(projectRoot, baseUrl, importPath));
...
var validateImportPath = ({
  ...
  return import_micromatch2.default.every(importPath, newPattern);
});
  ...
  if (!importPathExists) {
    ...
    if (isExternal) {
      const isValidExternalImportPattern = allowImportsFromExtracted.some(
        (p) => import_micromatch3.default.every(importPath, p)
      );
...
var removeProjectRootFromPath = (path20, projectRootWithBaseUrl) => path20.replace(projectRootWithBaseUrl + import_path19.sep, "").replace(/\\/g, "/");
...
var handleCallExpression = ({
  ...
  if (node.callee.type === import_utils15.AST_NODE_TYPES.Identifier && node.callee.name === "require" && node.arguments[0].type === import_utils15.AST_NODE_TYPES.Literal && typeof node.arguments[0].value === "string" || node.callee.type === import_utils15.AST_NODE_TYPES.MemberExpression && node.callee.object.type === import_utils15.AST_NODE_TYPES.Identifier && node.callee.object.name === "jest" && node.callee.property.type === import_utils15.AST_NODE_TYPES.Identifier && (node.callee.property.name === "mock" || node.callee.property.name === "requireActual") && node.arguments[0].type === import_utils15.AST_NODE_TYPES.Literal && typeof node.arguments[0].value === "string") {
...
var handleImportExpression = ({
  ...
  if (node.source.type !== import_utils16.AST_NODE_TYPES.Literal || typeof node.source.value !== "string")
    return;
  const importPath = node.source.value;
  validateImport({ importPath, context, node, config });
};
...
// src/rules/independentModules/independentModules.ts
var independentModules = import_utils17.ESLintUtils.RuleCreator(
